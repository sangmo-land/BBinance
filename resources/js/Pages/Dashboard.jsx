          ...
          {isAdmin && groupedUsers ? (
            <div className="space-y-12">
              {groupedUsers.data.map((user) => (
                <div key={user.id} className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300">
                        ...
                        {user.accounts.map((account) => (
                          ...
                          >
                            <div className={`absolute top-0 left-0 w-full h-1 ${
                                ...
                                account.account_type === 'crypto' ? 'bg-gradient-to-r from-amber-400 to-orange-600' : 
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-3">
                                  <span className={`text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full ${
                                      account.account_type === 'fiat' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 
                                      account.account_type === 'crypto' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 
                                      'bg-blue-50 text-blue-700 border border-blue-100'
                                  }`}>
                                      {account.account_type ?? 'Account'} wallet
                                  </span>
                                  <span className={`w-2 h-2 rounded-full ${account.is_active ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-gray-300'}`}></span>
                                </div>
                                <p className="text-lg font-mono font-bold text-gray-900 tracking-tight">{account.account_number}</p>
                              </div>
                              <div className={`p-3 rounded-xl ${
                                account.account_type === 'fiat' ? 'bg-emerald-50 text-emerald-600' :
                                account.account_type === 'crypto' ? 'bg-amber-50 text-amber-600' :
                              ...
                              {account.account_type === 'fiat' ? (
                                  <>
                                      <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">USD Balance</p>
                                          <p className="text-xl font-black text-gray-900">
                                              ${formatNumber(account.balances?.filter(b => b.currency === 'USD').reduce((sum, b) => sum + Number(b.balance), 0) ?? 0, 2)}
                                          </p>
                                      </div>
                                      <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">EUR Balance</p>
                                          <p className="text-xl font-black text-gray-900">
                                  ...
                                  <>
                                      <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Total BTC</p>
                                          <p className="text-xl font-black text-gray-900">
                                              {formatNumber(account.total_btc_value ?? 0, 8)} <span className="text-sm font-bold text-gray-400">BTC</span>
                                          </p>
                                      </div>
                                      <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">USDT Value</p>
                                          <p className="text-xl font-black text-gray-900">
                                              {formatNumber(account.total_usdt_value ?? 0, 2)} <span className="text-sm font-bold text-gray-400">USDT</span>
                                          </p>
                                      </div>
                                  </>
                              ) : (
                                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 text-center">
                                      ...
                                      <span className="text-lg font-bold text-gray-500 ml-2">{account.currency}</span>
                                      </p>
                                  </div>
                              )}
